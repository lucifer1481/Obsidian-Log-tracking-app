...
import com.axiel7.lucifer.data.model.manga.MangaList
...
import com.axiel7.lucifer.ui.composables.BackIconButton
import com.axiel7.lucifer.ui.composables.OnBottomReached
import com.axiel7.lucifer.ui.composables.media.MediaItemDetailed
...
import com.axiel7.lucifer.utils.NumExtensions.toStringPositiveValueOrNull
import com.axiel7.lucifer.utils.NumExtensions.toStringPositiveValueOrUnknown
...
private fun SearchViewContent(
    ...
    // 🚀 Updated to check both MAL items and External items for placeholders
    val shouldShowPlaceholder = query.isNotBlank() && uiState.mediaList.isEmpty() && uiState.externalMediaList.isEmpty()
    val shouldShowSearchHistory = (query.isBlank() || !uiState.noResults) &&
            uiState.mediaList.isEmpty() && uiState.externalMediaList.isEmpty() && !uiState.isLoading
    ...
    // 🚀 Removed the "Integration coming soon" toast blocker!
    @Composable
    fun FilterRow() {
        Row(
            modifier = Modifier
            ...
            MediaType.searchValues.forEach { mediaType ->
                FilterChip(
                    selected = uiState.mediaType == mediaType,
                    onClick = {
                        event?.onChangeMediaType(mediaType)
                    },
                    label = { Text(text = mediaType.localized()) },
                    modifier = Modifier.padding(end = 8.dp),
                    leadingIcon = {
                        if (uiState.mediaType == mediaType) {
                            Icon(
    ...
    // Existing ItemView for MAL Anime/Manga
    @Composable
    ...
    // 🚀 NEW: View to render our TMDB/RAWG items and tap to add to Library!

    @Composable
    fun ExternalItemView(item: CloudMedia) {
        MediaItemDetailed(
            title = item.title,
            ...
            subtitle1 = {
                Text(
                    text = "Via ${if (item.mediaType == "GAMES") "RAWG" else "TMDB"} Database",
                    color = MaterialTheme.colorScheme.onSurfaceVariant
                )
            },
            subtitle2 = {},
            subtitle3 = {},
            // 🚀 FIXED: Now this opens the Details Page instead of instantly saving!
            onClick = dropUnlessResumed {
    ...
    if (!isCompactScreen) {
        ...
        LazyVerticalGrid(
            ...
            // 🚀 Logic to display the correct list based on what tab is selected
            if (uiState.mediaType == MediaType.ANIME || uiState.mediaType == MediaType.MANGA) {
                ...
                items(
                    items = uiState.externalMediaList,
                    key = { it.apiId }
                ) { item ->
                    ExternalItemView(item = item)
                }
            }
